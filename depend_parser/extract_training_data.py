...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack) == 0:
            res[0] = self.word_vocab["<NULL>"]
            res[1] = self.word_vocab["<NULL>"]
            res[2] = self.word_vocab["<NULL>"]

        elif len(state.stack) == 1:
            res[0] = self.word_vocab["<ROOT>"]
            res[1] = self.word_vocab["<NULL>"]
            res[2] = self.word_vocab["<NULL>"]
            
        elif len(state.stack) == 2:
            if pos[state.stack[-1]] == "CD":
                res[0] = self.word_vocab["<CD>"]
            elif pos[state.stack[-1]] == "NNP":
                res[0] = self.word_vocab["<NNP>"]
            elif words[state.stack[-1]] not in self.word_vocab.keys():
                res[0] = self.word_vocab["<UNK>"]
            else:
                res[0] = self.word_vocab[words[state.stack[-1]]]

            res[1] = self.word_vocab["<ROOT>"]
            res[2] = self.word_vocab["<NULL>"]
            ...
            # res[1]
            if pos[state.stack[-2]] == "CD":
                res[0] = self.word_vocab["<CD>"]
            elif pos[state.stack[-2]] == "NNP":
                res[0] = self.word_vocab["<NNP>"]
            elif words[state.stack[-2]] not in self.word_vocab.keys():
                res[0] = self.word_vocab["<UNK>"]
            else:
                res[0] = self.word_vocab[words[state.stack[-2]]]

            # res[2]
            res[2] = self.word_vocab["<ROOT>"]

        # buffer
        if len(state.buffer) == 0:
            res[3] = self.word_vocab["<NULL>"]
            res[4] = self.word_vocab["<NULL>"]
            res[5] = self.word_vocab["<NULL>"]

        elif len(state.buffer) == 1:
            if state.buffer[-1] == 0:
                res[3] = self.word_vocab["<ROOT>"]
            elif pos[state.buffer[-1]] == "CD":
                res[3] = self.word_vocab["<CD>"]
            elif pos[state.buffer[-1]] == "NNP":
                res[3] = self.word_vocab["<NNP>"]
            elif words[state.buffer[-1]] not in self.word_vocab.keys():
                res[3] = self.word_vocab["<UNK>"]
            else:
                res[3] = self.word_vocab[words[state.buffer[-1]]]
            ...
            if state.buffer[-2] == 0:
                res[4] = self.word_vocab["<ROOT>"]
            elif pos[state.buffer[-2]] == "CD":
                res[4] = self.word_vocab["<CD>"]
            elif pos[state.buffer[-2]] == "NNP":
                res[4] = self.word_vocab["<NNP>"]
            elif words[state.buffer[-2]] not in self.word_vocab.keys():
                res[4] = self.word_vocab["<UNK>"]
            else:
                res[4] = self.word_vocab[words[state.buffer[-2]]]
            ...
            if state.buffer[-3] == 0:
                res[5] = self.word_vocab["<ROOT>"]
            elif pos[state.buffer[-3]] == "CD":
                res[5] = self.word_vocab["<CD>"]
            elif pos[state.buffer[-3]] == "NNP":
                res[5] = self.word_vocab["<NNP>"]
            elif words[state.buffer[-3]] not in self.word_vocab.keys():
                res[5] = self.word_vocab["<UNK>"]
            else:
                res[5] = self.word_vocab[words[state.buffer[-3]]]
    ...
    def get_output_representation(self, output_pair):  
        ...
        if output_pair[0] == "left_arc":
            ind = dep_relations.index((output_pair[1]))
            res[ind] = 1
        elif output_pair[0] == "right_arc":
            ind = dep_relations.index((output_pair[1])) + 45
            res[ind] = 1
        else:
            res[90] = 1
        ...
        res = keras.utils.to_categorical(ind, 91)
...
if __name__ == "__main__":
    ...
    with open("/Users/apple/Desktop/semester_1/5.nlp/hw/hw3/data/train.conll", 'r') as in_file: 
        ...
        np.save("/Users/apple/Desktop/semester_1/5.nlp/hw/hw3/data/input_train.npy", inputs)
        np.save("/Users/apple/Desktop/semester_1/5.nlp/hw/hw3/data/target_train.npy", outputs)
